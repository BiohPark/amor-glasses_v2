...
class Training:
        ...
        try:
            for i, pattern in enumerate(pattern_names):
                ...
                for j in range(data_cnt):
                    tmp_data = pd.read_csv(data_list[j], header=None).iloc[1:, :]
    ...
    def get_model_s(self, MAX_SEQ=31, num_patterns=4, num_cells=8, filter1=64, filter2=128):
        model = Sequential()
        model.add(Masking())
        model.add(LSTM(num_cells))
        model.add(Dropout(0.2))

        model.add(Conv1D(filter1, 8, padding="same", kernel_initializer="he_uniform"))
        model.add(BatchNormalization())

    def get_model(self, MAX_SEQ=31, num_sensors=5, num_patterns=4, num_cells=8):
        print("Getting Model")
...
def training_model(model, X, y, epochs=50, batch_size=32, validation_split=2/9, verbose=1):
    global version
    ...
    history = model.fit(X, y, epochs=epochs, batch_size=batch_size, validation_split=validation_split,
                verbose=verbose, callbacks=get_callback(model_name))
...
def get_learning_curve(train_history):
    plt.figure(1)
